...
exports.init = function(grunt){
  ...
  function build(ChromeExtension, callback) {
    async.series([
      ...
      ChromeExtension.load.bind(ChromeExtension),
      function removeExcluded(done){
        async.each(
          ChromeExtension._excludedFiles.map(function(f){
            return path.join(ChromeExtension.path, f);
          }), rm, done
        );
      },
      function packExtension(done){
        ChromeExtension.pack(function(err, data){
          if (err){
            grunt.fail.fatal(err);
          }

          fs.writeFile(this.dest, data, done);
        });
      }
    ], callback);
  }
  ...
  function resolveIgnoredFiles(sourceDir, src){
    return _
      .difference(grunt.file.expand(sourceDir + '/**/*'), src)
      .map(function(file){
  ...
  function expandConfiguration(taskConfig, defaults){
    ...
    if (!path.extname(taskConfig.dest)){
      var filename = taskConfig.options.filename || filename_template;

      taskConfig.dest = path.join(taskConfig.dest, filename);
    }

    taskConfig.dest = grunt.template.process(taskConfig.dest, {
      "data": {
